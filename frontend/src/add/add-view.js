  ...
  async _submit(event) {
    ...
    try {
      const payload = {
        ...
        capabilities: [
          ...(this._preset.suggested_capabilities ?? []),
        ],
      };

      if (this._code.trim()) {
        payload.code = this._code.trim();
      }

      if (this._areaId) {
        payload.area_id = this._areaId;
      }

      const response = await createBindHomeApi(
        this.hass,
      ).createAssetsBulk([payload]);
      ...
      // The backend write is now known to be committed.
      // A later refresh failure must never make this request retryable.
      ...
      ${!this._preset
        ...
        : html`<form class="form surface" @submit=${this._submit}>
            ...
            <div class="fields">
              ...
              ><label
                ...
                >
                  ...
                  ${this.areas.map(
                    (area) =>
                      html`<option value=${area.area_id} ?selected=${area.area_id === this._areaId}>${area.name}</option>`,
                  )}
                </select></label
              >
            </div>
            ${this._error
              ...
              : nothing}
            ${this._sync
              ? html`<div class="success" role="status">${this._sync}</div>`
              : nothing}
            <div class="actions">
              <button
                type="button"
