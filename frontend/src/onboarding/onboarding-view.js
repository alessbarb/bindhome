...
export class BindHomeOnboardingView extends LitElement {
  ...
  _complete(startInventory) {
    this.dispatchEvent(new CustomEvent("onboarding-complete", { detail: { startInventory }, bubbles: true, composed: true }));
  }

  _renderWelcome() {
    return html`<p class="eyebrow">${this.t("onboarding.welcome_eyebrow")}</p><h1>${this.t("onboarding.welcome_title")}</h1><p class="lead">${this.t("onboarding.welcome_body")}</p><div class="example"><div class="example-row"><div class="example-box"><strong>${this.t("onboarding.stable_title")}</strong><span class="muted">${this.t("onboarding.stable_example")}</span></div><span class="arrow" aria-hidden="true">→</span><div class="example-box"><strong>${this.t("onboarding.replaceable_title")}</strong><span class="muted">${this.t("onboarding.replaceable_example")}</span></div></div></div>`;
  }
  _renderModel() {
    const rows = ["asset", "capability", "binding", "representation"];
    return html`<p class="eyebrow">${this.t("onboarding.model_eyebrow")}</p><h1>${this.t("onboarding.model_title")}</h1><p class="lead">${this.t("onboarding.model_body")}</p><div class="model">${rows.map((row, index) => html`<div class="model-row"><span class="number">${index + 1}</span><div><strong>${this.t(`onboarding.${row}_title`)}</strong><span class="muted">${this.t(`onboarding.${row}_body`)}</span></div></div>`)}</div>`;
  }
  _renderStructure() {
    return html`<p class="eyebrow">${this.t("onboarding.structure_eyebrow")}</p><h1>${this.t("onboarding.structure_title")}</h1><p class="lead">${this.t("onboarding.structure_body")}</p><div class="structure"><div class="counts"><div class="count"><strong>${this.floors.length}</strong><span class="muted">${this.t("onboarding.floors_detected")}</span></div><div class="count"><strong>${this.areas.length}</strong><span class="muted">${this.t("onboarding.areas_detected")}</span></div></div>${this.areas.length === 0 ? html`<div class="warning">${this.t("onboarding.no_areas")}</div>` : html`<p class="muted">${this.t("onboarding.structure_ready")}</p>`}</div>`;
  }
  _renderStart() {
    return html`<p class="eyebrow">${this.t("onboarding.start_eyebrow")}</p><h1>${this.t("onboarding.start_title")}</h1><p class="lead">${this.t("onboarding.start_body")}</p><div class="next-steps"><strong>${this.t("onboarding.after_inventory_title")}</strong><ol><li>${this.t("onboarding.after_inventory_binding")}</li><li>${this.t("onboarding.after_inventory_topology")}</li><li>${this.t("onboarding.after_inventory_representation")}</li></ol></div>`;
  }

  render() {
    const views = [() => this._renderWelcome(), () => this._renderModel(), () => this._renderStructure(), () => this._renderStart()];
    return html`<div class="page">
      <div class="progress" aria-label=${this.t("onboarding.progress_label")}>${views.map((_, index) => html`<span class=${index <= this._step ? "active" : ""}></span>`)}</div>
      ${views[this._step]()}
      <div class="actions">
        ${this._step > 0 ? html`<button @click=${() => (this._step -= 1)}>${this.t("onboarding.back")}</button>` : null}
        ${this._step < views.length - 1
          ? html`<button class="primary" @click=${() => (this._step += 1)}>${this.t("onboarding.next")}</button>`
          : html`<button class="primary" ?disabled=${this.areas.length === 0} @click=${() => this._complete(false)}>${this.t("nav.home")}</button>`}
        <button class="skip" @click=${() => this._complete(false)}>${this.t("onboarding.skip")}</button>
      </div>
    </div>`;
  }
}
