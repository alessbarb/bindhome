...
- Runtime Binding resolution now prefers persisted Home Assistant Entity Registry identity and resolves it to the current `entity_id`, so ordinary entity-id renames keep working without changing the BindHome Binding. Missing stable Registry entries fail closed instead of falling back to a possibly reused old entity id; state-machine-only Bindings retain their documented `entity_id` fallback. ([#51](https://github.com/alessbarb/bindhome/issues/51))
