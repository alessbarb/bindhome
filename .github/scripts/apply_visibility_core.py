...
replace(
    ...
    "REGISTRY_SCHEMA_VERSION: Final = 3",
)

models = Path("custom_components/bindhome/models.py")
text = models.read_text(encoding="utf-8")
anchor = "\n\n@dataclass(frozen=True, slots=True)\nclass Relation:"
if anchor not in text:
    raise SystemExit("HardwareAdoption insertion anchor missing")
adoption_model = '''
...
'''
models.write_text(text.replace(anchor, adoption_model + anchor, 1), encoding="utf-8")
...
replace(
    "custom_components/bindhome/migrations.py",
...
replace(
    "custom_components/bindhome/__init__.py",
...
init_path = Path("custom_components/bindhome/__init__.py")
init_text = init_path.read_text(encoding="utf-8")
if "async def async_remove_entry(" not in init_text:
...
adoption_path = Path("custom_components/bindhome/adoption.py")
...
migrations_test = Path("tests/test_registry_migrations.py")
test = migrations_test.read_text(encoding="utf-8")
test = test.replace(
...
test = test.replace('    assert result.target_version == 2\n', '    assert result.target_version == 3\n', 1)
test = test.replace('    assert result.canonical_payload["schema_version"] == 2\n', '    assert result.canonical_payload["schema_version"] == 3\n', 1)
test = test.replace('    assert result.target_version == 2\n', '    assert result.target_version == 3\n', 1)
test = test.replace('    assert result.canonical_payload["schema_version"] == 2\n', '    assert result.canonical_payload["schema_version"] == 3\n', 1)
test = test.replace(
...
migrations_test.write_text(test, encoding="utf-8")

identity_test = Path("tests/test_binding_target_identity.py")
...
Path("tests/test_adoption.py").write_text('''"""Tests for reversible hardware visibility adoption."""
...
Path(".github/scripts/apply_visibility_core.py").unlink()
Path(".github/workflows/visibility-core-apply.yml").unlink()
