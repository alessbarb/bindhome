...
export class BindHomePanel extends LitElement {
  ...
  render() {
    ...
    else if (this._view === "inventory") content = html`<bindhome-inventory-workflow .hass=${this.hass} .t=${t} .presets=${this._presets} .floors=${this._floors} .areas=${this._areas} .assets=${this._assets} @assets-refreshed=${this._assetsRefreshed} @view-infrastructure=${() => (this._view = "infrastructure")}></bindhome-inventory-workflow>`;
    else content = html`<bindhome-infrastructure-inspector .t=${t} .registry=${this._registry ?? {}} .areas=${this._areas}></bindhome-infrastructure-inspector>`;
