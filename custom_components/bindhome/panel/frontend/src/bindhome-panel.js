...
export class BindHomePanel extends LitElement {
  static properties = {
    ...
    _floors: { state: true }, _areas: { state: true }, _assets: { state: true }, _registry: { state: true },
  };
  constructor() { super(); this._view = "inventory"; this._loading = true; this._error = null; this._presets = []; this._floors = []; this._areas = []; this._assets = []; this._registry = null; this._loadedForHass = null; }
  static styles = css`
  ...
  `;
  updated(changed) { if (changed.has("hass") && this.hass && this.hass !== this._loadedForHass) this._load(); }
  async _load() {
    this._loadedForHass = this.hass; this._loading = true; this._error = null;
    const bindhome = createBindHomeApi(this.hass); const ha = createHomeAssistantApi(this.hass);
    try {
      const [presets, assets, registry, floors, areas] = await Promise.all([bindhome.listPresets(), bindhome.listAssets(), bindhome.getRegistry(), ha.listFloors(), ha.listAreas()]);
      this._presets = presets; this._assets = assets; this._registry = registry; this._floors = floors; this._areas = areas;
    } catch (error) { this._error = error?.message || "BindHome could not load its inventory data."; }
  ...
  render() {
    ...
    else if (this._error) content = html`<div class="state"><div class="state-content"><h2>BindHome could not load</h2><p>${this._error}</p><button class="retry" @click=${this._load}>Retry</button></div></div>`;
    ...
    return html`<div class="shell"><header><div class="brand"><ha-icon icon="mdi:home-switch"></ha-icon><h1>BindHome</h1></div><button class="refresh" aria-label="Refresh BindHome data" @click=${this._load} ?disabled=${this._loading}><ha-icon icon="mdi:refresh"></ha-icon></button></header><nav aria-label="BindHome sections"><button class=${this._view === "inventory" ? "active" : ""} @click=${() => (this._view = "inventory")}>Inventory</button><button class=${this._view === "infrastructure" ? "active" : ""} @click=${() => (this._view = "infrastructure")}>Infrastructure</button></nav><main>${content}</main></div>`;
  }
}
